...
<main class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 lg:py-12">
    <header class="mb-8 flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
        <div class="max-w-2xl">
            ...
            <p class="text-muted-foreground mt-2 text-sm sm:text-base">Upload or download, inspect, and restore a database dump without exposing administrative credentials.</p>
        </div>
    ...
    <section class="overflow-hidden rounded-xl border bg-card shadow-xs" id="upload-card">
        <div class="border-b px-5 py-4">
            <h2 class="font-semibold">Provide database dump</h2>
            <p class="text-muted-foreground mt-1 text-sm">Upload from this device or let Vito download a direct URL. Accepted formats: .sql, .sql.gz, or a ZIP containing exactly one .sql file.</p>
        </div>
        <div class="p-5">
            <fieldset class="mb-4 grid gap-3 sm:grid-cols-2">
                <legend class="sr-only">Database dump source</legend>
                <label class="hover:bg-muted/30 flex cursor-pointer items-start gap-3 rounded-lg border p-4"><input class="accent-primary mt-0.5 size-4" type="radio" name="source-mode" value="file" checked><span><strong class="block text-sm">Upload a file</strong><span class="text-muted-foreground mt-1 block text-xs">Choose a dump stored on this device.</span></span></label>
                <label class="hover:bg-muted/30 flex cursor-pointer items-start gap-3 rounded-lg border p-4"><input class="accent-primary mt-0.5 size-4" type="radio" name="source-mode" value="url"><span><strong class="block text-sm">Download a URL</strong><span class="text-muted-foreground mt-1 block text-xs">Vito fetches the dump directly in its background queue.</span></span></label>
            </fieldset>
            <div id="file-source-wrap">
            ...
            <div class="hidden rounded-lg border p-5" id="url-source-wrap">
                <label class="text-sm font-medium" for="database-url">Direct {{ config('database-import.remote_download_require_https', true) ? 'HTTPS' : 'HTTP or HTTPS' }} download URL</label>
                <input class="field mt-2" type="url" id="database-url" maxlength="4096" placeholder="https://storage.example/backups/database.sql.gz" autocomplete="off" spellcheck="false">
                <p class="text-muted-foreground mt-2 text-xs">The URL must be publicly reachable, resolve to a public address, and return the complete file without requiring browser cookies. Redirects are checked separately.</p>
                @unless(config('database-import.remote_download_require_https', true))<p class="text-amber-700 dark:text-amber-400 mt-2 text-xs">HTTP downloads are not encrypted. Use only a short-lived link on a network you trust.</p>@endunless
                <p class="text-muted-foreground mt-1 text-xs">Up to {{ $maxUploadMb }} MB compressed · {{ $maxExtractedMb }} MB extracted</p>
            </div>
            ...
            <div class="mt-5 flex justify-end"><button id="upload-button" class="bg-primary text-primary-foreground hover:bg-primary/90 h-9 rounded-md px-4 text-sm font-medium shadow-xs transition-colors disabled:pointer-events-none disabled:opacity-50" type="button" disabled>Inspect source</button></div>
        </div>
    </section>
...
function sourceMode(){return document.querySelector('input[name="source-mode"]:checked').value;}
function syncSource(){const remote=sourceMode()==='url';$('file-source-wrap').classList.toggle('hidden',remote);$('url-source-wrap').classList.toggle('hidden',!remote);$('upload-progress-wrap').classList.add('hidden');$('upload-progress').style.width='0%';syncSourceButton();}
function syncSourceButton(){const scheme=CONFIG.limits.remoteRequireHttps?/^https:\/\//i:/^https?:\/\//i;const ready=sourceMode()==='url'?scheme.test($('database-url').value.trim()):!!selectedFile();$('upload-button').disabled=!ready;$('upload-button').textContent=sourceMode()==='url'?'Download and inspect':'Inspect upload';}
function fileChanged(){const file=selectedFile();$('file-title').textContent=file?file.name:'Drop your database dump here';$('file-subtitle').textContent=file?`${bytes(file.size)} selected`:'or choose a file from this device';syncSourceButton();}
function upload(){if(sourceMode()==='url')return downloadUrl();const file=selectedFile();if(!file)return;const form=new FormData();form.append('file',file);const xhr=new XMLHttpRequest();$('upload-button').disabled=true;$('upload-button').textContent='Inspecting…';$('upload-progress-wrap').classList.remove('hidden');xhr.open('POST',CONFIG.urls.uploads);xhr.setRequestHeader('Accept','application/json');xhr.setRequestHeader('X-CSRF-TOKEN',csrf);xhr.upload.onprogress=e=>{if(e.lengthComputable)$('upload-progress').style.width=`${Math.round(e.loaded/e.total*100)}%`;};xhr.onload=()=>{let data={};try{data=JSON.parse(xhr.responseText)}catch{}if(xhr.status<200||xhr.status>=300){message(data.message||`Upload failed (HTTP ${xhr.status}).`,true);syncSourceButton();return;}state.run=data;renderUpload();showOnly('destination-card');step(2);};xhr.onerror=()=>{message('The upload connection failed.',true);syncSourceButton();};xhr.send(form);}
async function downloadUrl(){const url=$('database-url').value.trim();if(!url)return;const button=$('upload-button');button.disabled=true;button.textContent='Queueing download…';$('upload-progress-wrap').classList.remove('hidden');try{state.run=await api(CONFIG.urls.uploads,{method:'POST',body:JSON.stringify({url})});$('file-subtitle').textContent=state.run.current_step;pollDownload();}catch(e){message(e.message,true);syncSourceButton();}}
function pollDownload(){clearInterval(state.poll);const tick=async()=>{try{state.run=await api(`${CONFIG.urls.runBase}/${state.run.id}`);$('upload-progress').style.width=`${state.run.progress||0}%`;$('upload-button').textContent=state.run.current_step||'Downloading…';if(state.run.status==='uploaded'){clearInterval(state.poll);renderUpload();showOnly('destination-card');step(2);}else if(state.run.status==='download_failed'){clearInterval(state.poll);message(state.run.error||'The remote download failed.',true);syncSourceButton();}}catch(e){clearInterval(state.poll);message(e.message,true);syncSourceButton();}};tick();state.poll=setInterval(tick,2000);}
...
$('database-file').addEventListener('change',fileChanged);$('database-url').addEventListener('input',syncSourceButton);document.querySelectorAll('input[name="source-mode"]').forEach(input=>input.addEventListener('change',syncSource));$('upload-button').addEventListener('click',upload);$('target-server').addEventListener('change',syncServer);$('target-database').addEventListener('change',syncCreateFields);$('target-user').addEventListener('change',syncCreateFields);$('review-button').addEventListener('click',review);$('start-button').addEventListener('click',start);$('change-button').addEventListener('click',()=>{showOnly('destination-card');step(2);});$('replace-upload-button').addEventListener('click',()=>location.reload());$('new-button').addEventListener('click',()=>location.reload());$('retry-button').addEventListener('click',async()=>{try{state.run=await api(`${CONFIG.urls.runBase}/${state.run.id}/retry`,{method:'POST'});renderRun(state.run);poll();}catch(e){message(e.message,true);if(e.data?.needs_review){showOnly('destination-card');step(2);document.querySelector('input[name="policy"][value="overwrite"]').checked=true;}}});$('cancel-button').addEventListener('click',async()=>{try{state.run=await api(`${CONFIG.urls.runBase}/${state.run.id}/cancel`,{method:'POST'});renderRun(state.run);clearInterval(state.poll);}catch(e){message(e.message,true);}});
